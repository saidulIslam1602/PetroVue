...
export const OffshorePlatformSimulation: React.FC = () => {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<PlatformData>(
    mockPlatformData[0]
  );
      ...
      setPlatforms(prev =>
        prev.map(platform => ({
          ...
          weather: {
            ...
            windSpeed: Math.max(
              0,
              platform.weather.windSpeed + (Math.random() - 0.5) * 2
            ),
            waveHeight: Math.max(
              0,
              platform.weather.waveHeight + (Math.random() - 0.5) * 0.5
            ),
            visibility: Math.min(
              10,
              Math.max(0, platform.weather.visibility + (Math.random() - 0.5))
            ),
            temperature:
              platform.weather.temperature + (Math.random() - 0.5) * 0.5,
            weatherWindow:
              platform.weather.windSpeed < 20 &&
              platform.weather.waveHeight < 4,
          },
          production: {
            ...platform.production,
            oilProduction: Math.max(
              0,
              platform.production.oilProduction + (Math.random() - 0.5) * 1000
            ),
            gasProduction: Math.max(
              0,
              platform.production.gasProduction + (Math.random() - 0.5) * 2000
            ),
            efficiency: Math.min(
              100,
              Math.max(
                70,
                platform.production.efficiency + (Math.random() - 0.5) * 0.5
              )
            ),
            pressure: Math.max(
              0,
              platform.production.pressure + (Math.random() - 0.5) * 2
            ),
            temperature: Math.max(
              0,
              platform.production.temperature + (Math.random() - 0.5)
            ),
          },
        }))
      );
  ...
  return (
    <Container maxWidth='xl' sx={{ py: 4 }}>
      <Typography variant='h4' gutterBottom sx={{ mb: 4, fontWeight: 'bold' }}>
        {norwegianTranslations.offshorePlatform}{' '}
        {norwegianTranslations.dashboard}
      </Typography>

      <Typography variant='subtitle1' sx={{ mb: 4, color: 'text.secondary' }}>
        {norwegianTranslations.realTime} overvåking av{' '}
        {norwegianTranslations.continentalShelf} operasjoner
      </Typography>
                  ...
                  <Chip
                    label={
                      platform.operational
                        ? norwegianTranslations.operational
                        : norwegianTranslations.maintenance
                    }
      ...
      >
          ...
          <CardContent>
            ...
            <Box sx={{ mt: 2 }}>
              <Typography variant='body2' sx={{ opacity: 0.9, mb: 1 }}>
                {norwegianTranslations.seaState}:{' '}
                {selectedPlatform.weather.seaState}/6
              </Typography>
              <LinearProgress
                variant='determinate'
                ...
                sx={{
                  ...
                  '& .MuiLinearProgress-bar': {
                    backgroundColor:
                      selectedPlatform.weather.seaState > 4
                        ? '#ef4444'
                        : '#10b981',
                  },
                }}
        ...
        <ProductionCard>
          <CardContent>
            ...
            <Box sx={{ mb: 2 }}>
              <Typography variant='body2' sx={{ opacity: 0.9, mb: 1 }}>
                {norwegianTranslations.efficiency}:{' '}
                {selectedPlatform.production.efficiency.toFixed(1)}%
              </Typography>
              <LinearProgress
                variant='determinate'
            ...
            >
              {selectedPlatform.production.oilProduction > 0 && (
                <Box sx={{ gridColumn: '1 / -1' }}>
                  ...
                  <Typography variant='h6'>
                    {selectedPlatform.production.oilProduction.toLocaleString(
                      'nb-NO'
                    )}{' '}
              ...
              <Box sx={{ gridColumn: '1 / -1' }}>
                ...
                <Typography variant='h6'>
                  {selectedPlatform.production.gasProduction.toLocaleString(
                    'nb-NO'
                  )}{' '}
                  Sm³/dag
                </Typography>
        ...
        <CardContent>
          ...
          >
            ...
            <Box sx={{ textAlign: 'center' }}>
              ...
              <Typography variant='body2' sx={{ opacity: 0.9 }}>
                {norwegianTranslations.personnel}{' '}
                {norwegianTranslations.onboard}
              </Typography>
            </Box>
            ...
            <Box sx={{ textAlign: 'center' }}>
              <Chip
                ...
                color={getSafetyStatusColor(
                  selectedPlatform.safety.emergencyStatus
                )}
                sx={{
                  fontSize: '1rem',
                  fontWeight: 'bold',
