...
export const ProductionTracker: React.FC<ProductionTrackerProps> = ({
  ...
  const generateProductionData = () => {
    const hours =
      selectedTimeframe === '24h' ? 24 : selectedTimeframe === '7d' ? 7 : 30;
    const data = [];
  ...
  const getOverallStatus = () => {
    ...
    const maintenanceWells = wells.filter(
      well => well.status === 'maintenance'
    ).length;
    ...
    <ProductionTrackerContainer className={className} data-testid={testId}>
      ...
      >
        ...
        <MetricCard
          ...
          status={
            metrics.activeWells === metrics.totalWells ? 'success' : 'warning'
          }
        />
      </div>
