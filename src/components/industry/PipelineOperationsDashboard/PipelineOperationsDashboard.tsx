  ...
  },
  {
    ...
    integrityData: {
      ...
      piggingData: {
        ...
        findings: ['Minor scale deposits', 'Corrosion within acceptable limits']
      }
    },
    leakDetection: {
...
export const PipelineOperationsDashboard: React.FC = () => {
  ...
  }), { totalFlow: 0, avgCapacity: 0, activePipelines: 0, totalLength: 0, totalAlerts: 0 });
  ...
  const getPipelineTypeIcon = (type: string) => {
    switch (type) {
      case 'oil': return <LocalGasStation />;
      case 'gas': return <Speed />;
      case 'water': return <WaterDrop />;
      case 'mixed': return <Opacity />;
  ...
  const getIntegrityStatus = (pipeline: PipelineData): { status: string; color: 'success' | 'warning' | 'error' } => {
    const wallThicknessRatio = pipeline.integrityData.wallThickness.current / pipeline.integrityData.wallThickness.minimum;
    const inspectionOverdue = new Date() > pipeline.integrityData.nextInspection;
    
    if (inspectionOverdue || wallThicknessRatio < 1.1) {
      return { status: 'Critical', color: 'error' };
    } else if (wallThicknessRatio < 1.3 || pipeline.integrityData.corrosionRate > 0.1) {
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Typography variant="subtitle1" sx={{ mb: 4, color: 'text.secondary' }}>
        {norwegianTranslations.realTime} overvåking av norsk rørledning infrastruktur
      </Typography>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            {pipelines.map((pipeline) => (
              <Chip
                ...
                color={selectedPipeline.pipelineId === pipeline.pipelineId ? 'primary' : 'default'}
                variant={selectedPipeline.pipelineId === pipeline.pipelineId ? 'filled' : 'outlined'}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  {selectedPipeline.route.length} km • {selectedPipeline.route.diameter}" • {selectedPipeline.route.material}
                </Typography>
              </Box>
              <Chip 
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 3, mb: 3 }}>
              <Paper sx={{ p: 2 }}>
                ...
                <Typography variant="h4" sx={{ fontWeight: 'bold', color: 'primary.main' }}>
                  {selectedPipeline.operatingData.flowRate.toLocaleString('nb-NO')}
                </Typography>
                ...
                <Typography variant="caption">
                  {selectedPipeline.operatingData.capacity.current.toFixed(1)}% av kapasitet
                </Typography>
              </Paper>
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <LinearProgress 
                  variant="determinate" 
                  value={(selectedPipeline.operatingData.pressure.operating / selectedPipeline.operatingData.pressure.maximum) * 100}
                  sx={{ mt: 1 }}
                  color="warning"
                />
                <Typography variant="caption">
              ...
              <Table size="small">
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell align="right">{selectedPipeline.operatingData.pressure.inlet}</TableCell>
                    <TableCell align="right">{selectedPipeline.operatingData.pressure.outlet}</TableCell>
                    <TableCell align="right">
                      {(selectedPipeline.operatingData.pressure.inlet - selectedPipeline.operatingData.pressure.outlet).toFixed(1)}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Temperatur (°C)</TableCell>
                    <TableCell align="right">{selectedPipeline.operatingData.temperature.inlet}</TableCell>
                    <TableCell align="right">{selectedPipeline.operatingData.temperature.outlet}</TableCell>
                    <TableCell align="right">
                      {(selectedPipeline.operatingData.temperature.inlet - selectedPipeline.operatingData.temperature.outlet).toFixed(1)}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Card>
            <CardContent>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <LinearProgress 
                  variant="determinate" 
                  value={(selectedPipeline.integrityData.wallThickness.current / selectedPipeline.integrityData.wallThickness.design) * 100}
                  color={getIntegrityStatus(selectedPipeline).color}
                  sx={{ mb: 1 }}
                />
                <Typography variant="caption">
                  {selectedPipeline.integrityData.wallThickness.current} mm av {selectedPipeline.integrityData.wallThickness.design} mm design
                </Typography>
              </Box>
              ...
              <Table size="small">
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell align="right">{selectedPipeline.integrityData.corrosionRate} mm/år</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Siste inspeksjon</TableCell>
                    <TableCell align="right">
                      {selectedPipeline.integrityData.lastInspection.toLocaleDateString('nb-NO')}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Neste inspeksjon</TableCell>
                    <TableCell align="right">
                      {selectedPipeline.integrityData.nextInspection.toLocaleDateString('nb-NO')}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
          ...
          <Card>
            <CardContent>
              ...
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ mb: 1 }}>System: {selectedPipeline.leakDetection.system}</Typography>
                <Typography variant="body2" sx={{ mb: 1 }}>
                  Følsomhet: {selectedPipeline.leakDetection.sensitivity} m³/h
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  Sist kalibrert: {selectedPipeline.leakDetection.lastCalibration.toLocaleDateString('nb-NO')}
                </Typography>
              </Box>

              {selectedPipeline.leakDetection.alerts.length > 0 ? (
                <Box>
                  <Typography variant="subtitle2" sx={{ mb: 1 }}>Aktive Varsler:</Typography>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 3 }}>
            <Paper sx={{ p: 2 }}>
              ...
              <Typography variant="body1" sx={{ fontWeight: 'bold', mb: 1 }}>
                {selectedPipeline.integrityData.piggingData.lastRun.toLocaleDateString('nb-NO')}
              </Typography>
              <Typography variant="body2">
                Funn: {selectedPipeline.integrityData.piggingData.findings.join(', ')}
              </Typography>
            </Paper>
            ...
            <Paper sx={{ p: 2 }}>
              ...
              <Typography variant="body1" sx={{ fontWeight: 'bold', mb: 1 }}>
                {selectedPipeline.integrityData.piggingData.nextScheduled.toLocaleDateString('nb-NO')}
              </Typography>
              <Typography variant="body2">
                {Math.ceil((selectedPipeline.integrityData.piggingData.nextScheduled.getTime() - Date.now()) / (1000 * 60 * 60 * 24))} dager igjen
              </Typography>
            </Paper>
            ...
            <Paper sx={{ p: 2 }}>
              ...
              <Typography variant="body1" sx={{ fontWeight: 'bold', mb: 1 }}>
                {new Date().getFullYear() - selectedPipeline.route.installationYear} år
              </Typography>
              <Typography variant="body2">
