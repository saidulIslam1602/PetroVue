...
export const CarbonFootprintCalculator: React.FC = () => {
  ...
  const calculateEmissions = useMemo((): EmissionData => {
    ...
    const flaringEmissions =
      parseFloat(inputs.flaringVolume || '0') * EMISSION_FACTORS.flaring;

    const ventingEmissions =
      parseFloat(inputs.ventingVolume || '0') * EMISSION_FACTORS.venting;
  ...
  const getRecommendations = () => {
    ...
    if (inputs.electricitySource !== 'renewable') {
      recommendations.push({
        ...
        description:
          'Transition to renewable energy sources can reduce emissions by up to 98%',
      });
    }
    ...
    if (parseFloat(inputs.flaringVolume || '0') > 0) {
      recommendations.push({
        ...
        description:
          'Implement flare gas recovery systems to capture and utilize gas',
      });
    }
    ...
    if (parseFloat(inputs.ventingVolume || '0') > 0) {
      recommendations.push({
        ...
        description:
          'Methane venting has 28x global warming potential - capture or combust',
      });
    }
    ...
    recommendations.push({
      ...
      description:
        'Upgrade equipment and optimize processes for 15% efficiency gain',
    });
  ...
  const totalPotentialReduction = recommendations.reduce(
    (sum, rec) => sum + rec.reduction,
    0
  );
    ...
    <CalculatorContainer>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant='subtitle1' color='text.secondary'>
          Calculate emissions across Scope 1, 2, and 3 with actionable reduction
          recommendations
        </Typography>
      </Box>
          ...
          >
            ...
            <Box>
              ...
              <Typography variant='body2' sx={{ opacity: 0.8 }}>
                kg CO₂e (
                {(
                  (totalPotentialReduction / calculateEmissions.total) *
                  100
                ).toFixed(1)}
                %)
              </Typography>
      ...
      >
        <EmissionCard>
          ...
          >
            {(
              (calculateEmissions.scope1 / calculateEmissions.total) *
              100
            ).toFixed(1)}
            % of total
          </Typography>
        ...
        <EmissionCard>
          ...
          >
            {(
              (calculateEmissions.scope2 / calculateEmissions.total) *
              100
            ).toFixed(1)}
            % of total
          </Typography>
        ...
        <EmissionCard>
          ...
          >
            {(
              (calculateEmissions.scope3 / calculateEmissions.total) *
              100
            ).toFixed(1)}
            % of total
          </Typography>
            ...
            <Box>
              ...
              <Alert severity='info' sx={{ mt: 2 }}>
                <Typography variant='body2'>
                  All calculations use standard emission factors from GHG
        ...
        <TabPanel value={activeTab} index={1}>
          ...
          <Alert severity='success' sx={{ mt: 3 }}>
            ...
            <Typography variant='body2'>
              ...
              <strong>
                {totalPotentialReduction.toLocaleString()} kg CO₂e
              </strong>{' '}
              (
              {(
                (totalPotentialReduction / calculateEmissions.total) *
